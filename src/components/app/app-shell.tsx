      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        <header className="h-16 shrink-0 border-b border-hairline/70 flex items-center gap-3 px-4 md:px-8 bg-background/70 backdrop-blur-md sticky top-0 z-10">
          <div className="md:hidden flex items-center gap-2">
            <div className="h-8 w-8 rounded-xl flex items-center justify-center text-white text-xs font-bold" style={{ background: "var(--gradient-magenta)" }}>
              C
            </div>
            <div className="text-sm font-semibold">CoreEgin</div>
          </div>
          <div className="hidden md:flex items-center gap-2 max-w-sm w-full text-muted-foreground text-sm px-3 py-2 rounded-lg bg-card/70 border border-hairline hover:border-primary/30 hover:bg-card transition-all cursor-text">
        ...
        <nav className="md:hidden border-t border-hairline grid grid-cols-4 bg-sidebar/95 backdrop-blur">
          {NAV.map((item) => {
            const active = pathname === item.to || pathname.startsWith(item.to + "/");
            const Icon = item.icon;
            const showBadge = item.to === "/chat" && unread > 0;
